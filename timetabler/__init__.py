...
from flask_principal import ActionNeed, Permission, Principal, RoleNeed, identity_loaded
...
app.config.update(SECRET_KEY=appcfg["secretkey"])
db = SQLAlchemy(app)
...
def current_privileges():
    return (
        ("{method} : {value}").format(method=n.method, value=n.value)
        for n in apps_needs
        if n in g.identity.provides
    )
...
try:
    ...
    logger.error(f"FAILED with traceback {traceback.format_exc()}... Rolling Back")
    logger.error()
    logger.error(
        "If you are running first-time setup, comment out line 87 from timetabler/__init__.py"
    )
    logger.error("Then, un-comment it when running runserver")
    logger.error()
    db.session.rollback()
