...
from wtforms import DateTimeField, IntegerField, PasswordField, SelectField, StringField
...
class EditStudentForm(NameForm):
    ...
    university = SelectField("University", validators=[Optional()], choices=[("", "")])
    college = SelectField("College", validators=[Optional()], choices=[("", "")])
...
class StudentForm(NameForm):
    ...
    university = SelectField(
        "University", validators=[Optional()], choices=[(-1, "")], coerce=int
    )
    college = SelectField(
        "College", validators=[Optional()], choices=[(-1, "")], coerce=int
    )
