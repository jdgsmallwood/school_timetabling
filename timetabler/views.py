...
from timetabler.models import Tutor, Subject
        ...
        if form.validate_on_submit():
            ...
            if success:
                ...
                return render_template(
                    "login.html", form=form, msg="Username or Password was incorrect"
                )
        else:
            return render_template(
                "login.html", form=form, msg="Please enter username and password."
            )
    ...
    if User.query.filter_by(username=user_id).first() is not None:
        ...
        if bcrypt.check_password_hash(user.password, password):
            ...
            identity_changed.send(
                current_app._get_current_object(), identity=Identity(user.username)
            )
            return True
        else:
            return False
...
def run_timetabler():
    return render_template(
        "runtimetabler.html", tutors=Tutor.get_all(), timeslots=Timeslot.get_all()
    )
...
def admin():
    return render_template(
        "admin.html", admin=getadmin(), timetables=Timetable.get_all()
    )
...
def delete_all_timetabled_classes():
    timetabledclasses = TimetabledClass.query.filter_by(
...
def managetutoravailability():
    return render_template(
        "tutoravailability.html", timeslots=Timeslot.get_all(), tutors=Tutor.get_all()
    )
...
def viewtimetable_ajax():
    data = (
        TimetabledClass.query.filter_by(
            year=get_current_year(), studyperiod=get_current_studyperiod()
        )
        .options(joinedload(TimetabledClass.tutor), joinedload(TimetabledClass.room))
        .all()
    )
    data2 = []
...
def viewtutors_ajax():
    data = (
        ...
        .options(joinedload(Tutor.subjects), joinedload(Tutor.availabletimes))
        .all()
    )
    data2 = []
    for row in data:
...
@admin_permission.require()
def viewusers_ajax():
    data = User.query.options(joinedload(User.tutor)).all()
    data2 = []
    for row in data:
        data2.append(row.__dict__)
    for row in data2:
...
def vieweligiblesubjects_ajax():
    data = (
        Subject.query.options(joinedload(Subject.students))
        .filter(
            Subject.year == get_current_year(),
            Subject.studyperiod == get_current_studyperiod(),
            Subject.tutor == None,
...
def num_eligible_subjects_mapped():
    ...
    eligiblesubjects = [
        subject for subject in allsubjects if len(subject.students) >= 3
    ]
    data = {}
    ...
    for timeslot in timeslots:
        ...
        for timeclass in timeslot.timetabledclasses:
            for student in timeclass.students:
                ...
                clashestimeslot[student.id]["subjects"].append(
