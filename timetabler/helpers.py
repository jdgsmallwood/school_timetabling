...
def runtimetable_with_rooms_two_step(
    ...
    logger.info("Assignment Variables")
    ...
    logger.info("Subject Variables")
    subject_vars = LpVariable.dicts(
        "SubjectVariables",
        [(j, k, m) for m in TEACHERS for j in TEACHERMAPPING[m] for k in TIMES],
        0,
        1,
        LpBinary,
    )
    ...
    # c
    logger.info("9:30 classes")
    num930classes = LpVariable.dicts(
        "930Classes", [(i) for i in TIMES], lowBound=0, cat=LpInteger
    )
    # w
    logger.info("Days for teachers")
    daysforteachers = LpVariable.dicts(
        "numdaysforteachers",
    ...
    # p
    daysforteacherssum = LpVariable.dicts(
        "numdaysforteacherssum", [(i) for i in TEACHERS], 0, cat=LpInteger
    )
    # variables for student clashes
    studenttime = LpVariable.dicts(
        "StudentTime",
    ...
    studentsum = LpVariable.dicts(
        "StudentSum", [(i) for i in STUDENTS], 0, cat=LpInteger
    )

    projectortime = LpVariable.dicts(
        "ProjectorSum", [(k) for k in TIMES], cat=LpInteger
    )
    projectorpositive = LpVariable.dicts(
        "ProjectorPositivePart", [(k) for k in TIMES], 0, cat=LpInteger
    )
    # Count the days that a teacher is rostered on. Make it bigger than a small number times the sum
    # for that particular day.
    for m in TEACHERS:
        logger.info("Counting Teachers for " + m)
    ...
    for m in TEACHERS:
        model += daysforteacherssum[(m)] == lpSum(
            daysforteachers[(m, d)] for d in range(len(day))
        )
    ...
    for k in TIMES:
        model += lpSum(
            subject_vars[(j, k, m)] for m in TEACHERS for j in TEACHERMAPPING[m]
        ) <= len(ROOMS)
    ...
    for m in TEACHERS:
        for j in TEACHERMAPPING[m]:
            for k in TIMES:
                model += (
                    lpSum(assign_vars[(i, j, k, m)] for i in SUBJECTMAPPING[j])
                    >= minclasssize * subject_vars[(j, k, m)]
                )
                model += (
                    lpSum(assign_vars[(i, j, k, m)] for i in SUBJECTMAPPING[j])
                    <= maxclasssize
    ...
    # Solving the model
    model += (
        100 * lpSum(studentsum[(i)] for i in STUDENTS)
        + lpSum(num930classes[(i)] for i in TIMES)
        + 500 * lpSum(daysforteacherssum[(m)] for m in TEACHERS)
        ...
        for j in TEACHERMAPPING[m]:
            for k in TIMES:
                if subject_vars[(j, k, m)].varValue == 1:
                    classpop[(j, k, m)] = sum(
                        assign_vars[(i, j, k, m)].varValue for i in SUBJECTMAPPING[j]
                    )
    ...
    if LpStatus[model.status] == "Optimal":
        ...
        teacher_number_rooms = LpVariable.dicts(
            "NumberRoomsTeacher",
            [(m, n) for m in TEACHERS for n in ROOMS],
            0,
            1,
            LpBinary,
        )
        teacher_number_rooms_sum = LpVariable.dicts(
            "NumberRoomsTeacherSum", [(m) for m in TEACHERS], 0
        )

        projector_rooms_sum = LpVariable.dicts(
            "ProjectorRooms", [(j) for j in PROJECTORS]
        )

        populationovershoot = LpVariable.dicts(
            "PopulationOvershoot", [(k, n) for k in TIMES for n in ROOMS]
        )

        poppositive = LpVariable.dicts(
            "PopulationPositivePart", [(k, n) for k in TIMES for n in ROOMS]
        )
        ...
        for m in TEACHERS:
            for j in TEACHERMAPPING[m]:
                for k in TIMES:
                    if (j, k, m) in classpop.keys():
                        model2 += (
                            lpSum(subject_vars_rooms[(j, k, m, n)] for n in ROOMS)
                            == subject_vars[(j, k, m)].varValue
                        )
        ...
        for k in TIMES:
            ...
            for n in ROOMS:
                ...
                model2 += populationovershoot[(k, n)] == (
                    lpSum(
        ...
        print("Setting Objective Function")
        model2 += (
            lpSum(teacher_number_rooms_sum[(m)] for m in TEACHERS)
            - 50 * lpSum(projector_rooms_sum[(j)] for j in PROJECTORS)
            + 10 * lpSum(poppositive[(k, n)] for k in TIMES for n in ROOMS)
        )
...
def preparetimetable(addtonewtimetable=False):
    ...
    (
        ...
        NONPREFERREDTIMES,
        CAPACITIES,
    ) = timetabler.models.get_timetable_data(rooms=True)

    print("Everything ready")
    executor.submit(
        runtimetable_with_rooms_two_step,
        STUDENTS,
        SUBJECTS,
        TIMES,
        day,
