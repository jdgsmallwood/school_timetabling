...
class CRUDMixin(db.Model):
    ...
    def get(cls, **kwargs):
        return cls.query.filter_by(
            year=get_current_year(), studyperiod=get_current_studyperiod(), **kwargs
        ).first()
...
tutoravailabilitymap = db.Table(
    "tutoravailabilitymap",
    db.Column("id", db.Integer, primary_key=True),
    db.Column("tutor_id", db.Integer, db.ForeignKey("tutors.id")),
    db.Column("timeslot_id", db.Integer, db.ForeignKey("timeslots.id")),
)
...
class Subject(Base):
    ...
    def view_subject_template(self, form, msg=""):
        return render_template(
            ...
            msg=msg,
            times=self.find_possible_times(),
...
class Student(Base):
    ...
    subjects = db.relationship(
        "Subject", secondary=substumap, backref=db.backref("students")
    )
    timetabledclasses = db.relationship(
        "TimetabledClass", secondary=stutimetable, backref=db.backref("students")
    )
    universityid = db.Column(db.Integer, db.ForeignKey("university.id"))
...
class Timetable(Base):
    ...
    timeslots = db.relationship(
        "Timeslot", single_parent=True, cascade="all,delete-orphan"
    )
...
class Tutor(Base):
    ...
    availabletimes = db.relationship(
        "Timeslot",
        secondary=tutoravailabilitymap,
...
class TimetabledClass(Base):
    ...
    subject = db.relationship(
        "Subject", backref=db.backref("timetabledclasses"), single_parent=True
    )
    ...
    tutor = db.relationship(
        "Tutor", backref=db.backref("timetabledclasses"), single_parent=True
    )
...
def get_student_template(student, form, msg):
    return render_template(
        "student.html",
        student=student,
        eligiblesubjects=Subject.get_all(),
...
def populate_students(df):
    ...
    for index, row in df.iterrows():
        if studyperiod == row[col_study_period]:
            student_name = (
                (
                    str(row[col_student_first_name])
                    + " "
                    + str(row[col_student_last_name])
                )
                .strip()
                .title()
            )
            print(
                "Adding: ", student_name, row[col_subject_code], row[col_subject_name]
            )
            student = Student.get_or_create(
                studentcode=str(int(row[col_student_id])),
                name=student_name,
                universityid=University.query.filter_by(name="University of Melbourne")
                .first()
                .id,
                collegeid=str(
                    College.query.filter_by(name="International House").first().id
                ),
            )
            subject = Subject.get_or_create(
                subcode=row[col_subject_code], subname=row[col_subject_name]
            )
...
def populate_timetabledata(df):
    ...
    for index, row in df.iterrows():
        ...
        timetabledclass = TimetabledClass.get_or_create(
            time=timeslot.id, subjectid=subject.id, timetable=timetable.id
        )
    ...
    for index, row in df.iterrows():
        ...
        for key in row.keys():
            ...
            if keysplit[0] in [
                ...
                timeslot = Timeslot.get_or_create(day=keysplit[0], time=keysplit[1])
                if row[key] == 1:
...
def populate_tutors(df):
    for index, row in df.iterrows():
        ...
        if subject not in tutor.subjects:
            print(
                "Assigning tutor: ", row["Tutor"], row["Subject Code"], row["Repeats"]
            )
...
def get_timetable_data(rooms=False):
    ...
    allsubjects = Subject.query.filter(
        Subject.year == get_current_year(),
...
def add_classes_to_timetable(
    TEACHERS, TEACHERMAPPING, SUBJECTMAPPING, TIMES, subject_vars, assign_vars, ROOMS
):
    print(ROOMS)
    for m in TEACHERS:
            ...
            for k in TIMES:
                for n in ROOMS:
                    ...
                    if subject_vars[(j, k, m, n)].varValue == 1:
                        timetabledclass = TimetabledClass.create(
...
def add_classes_to_timetable_twostep(
    TEACHERS,
    TEACHERMAPPING,
    SUBJECTMAPPING,
    TIMES,
    subject_vars_with_rooms,
    assign_vars,
    ROOMS,
    classpop,
):
...
def init_db_timeslots():
    ...
    for timeslot in timeslots:
        if (
            Timeslot.get(day=timeslot[0].split(" ")[0], time=timeslot[0].split(" ")[1])
            is None
        ):
            if timeslot[1] == False:
                Timeslot.create(
                    day=timeslot[0].split(" ")[0],
                    time=timeslot[0].split(" ")[1],
                    preferredtime=False,
                )
            else:
                Timeslot.create(
...
def init_db():
    logger.info("Importing study period and year")
    init_db_studyperiod()
    logger.info("Creating Timetable")
    init_db_timetable()
    logger.info("Creating admin user")
    init_db_users()
    logger.info("Creating University")
    init_db_uni()
    logger.info("Creating College")
    init_db_college()
    logger.info("Creating Timeslots")
    init_db_timeslots()
